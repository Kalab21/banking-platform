...
| Kafka deserialization | Type headers are off and the trusted-package list is the contract package alone, not `*`. A record names its type in an `eventType` field rather than a Java class the consumer would instantiate |
