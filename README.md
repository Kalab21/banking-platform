...
**Engineering proof:** 13 backend processes · 820 CI tests · idempotent money movement ·
...
| **Testing** | 820 tests in CI (JUnit 5, Mockito, Testcontainers, Vitest, Playwright), plus 34 live-stack Playwright scenarios and a PowerShell full-stack suite on demand |
...
| Evidence | Result |
|---|---:|
| Backend — unit, web-slice and Testcontainers integration | 538 |
...
The backend total is 508 unit and web-slice tests plus 30 integration tests that run
...
```bash
mvn -B --no-transfer-progress clean verify   # backend: 508 unit + 30 integration = 538
