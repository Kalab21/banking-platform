...
**Engineering proof:** 13 backend processes · 839 CI tests · idempotent money movement ·
...
| **Testing** | 839 tests in CI (JUnit 5, Mockito, Testcontainers, Vitest, Playwright), plus 34 live-stack Playwright scenarios and a PowerShell full-stack suite on demand |
...
| Evidence | Result |
|---|---:|
| Backend — unit, web-slice and Testcontainers integration | 557 |
...
The backend total is 512 unit and web-slice tests plus 45 integration tests that run
...
```bash
mvn -B --no-transfer-progress clean verify   # backend: 512 unit + 45 integration = 557
