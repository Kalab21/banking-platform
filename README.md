...
**Engineering proof:** 13 backend processes · 917 CI tests · idempotent money movement ·
...
| **Testing** | 917 tests in CI (JUnit 5, Mockito, Testcontainers, Vitest, Playwright), plus 34 live-stack Playwright scenarios and a PowerShell full-stack suite on demand |
...
| Evidence | Result |
|---|---:|
| Backend — unit, web-slice and Testcontainers integration | 635 |
...
The backend total is 547 unit and web-slice tests plus 88 integration tests that run
...
```bash
mvn -B --no-transfer-progress clean verify   # backend: 547 unit + 88 integration = 635
