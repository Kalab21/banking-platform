...
**Engineering proof:** 13 backend processes · 985 CI tests · idempotent money movement ·
...
| **Testing** | 985 tests in CI (JUnit 5, Mockito, Testcontainers, Vitest, Playwright), plus 34 live-stack Playwright scenarios and a PowerShell full-stack suite on demand |
...
| Evidence | Result |
|---|---:|
| Backend — unit, web-slice and Testcontainers integration | 703 |
...
The backend total is 569 unit and web-slice tests plus 134 integration tests that run
...
```bash
mvn -B --no-transfer-progress clean verify   # backend: 569 unit + 134 integration = 703
