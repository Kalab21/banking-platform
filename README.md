...
**Engineering proof:** 13 backend processes · 878 CI tests · idempotent money movement ·
...
| **Testing** | 878 tests in CI (JUnit 5, Mockito, Testcontainers, Vitest, Playwright), plus 34 live-stack Playwright scenarios and a PowerShell full-stack suite on demand |
...
| Evidence | Result |
|---|---:|
| Backend — unit, web-slice and Testcontainers integration | 596 |
...
The backend total is 526 unit and web-slice tests plus 70 integration tests that run
...
Publishing is no longer part of that problem for the services that have been
migrated: `account-service`, `transaction-service` and `application-service` write
the event to a transactional outbox in the same transaction as the change, and a
relay sends it afterwards, so a committed change cannot go unannounced. The
remaining producers still publish directly. Compensation across services is
separate work again — the outbox makes the event reliable, not the transfer atomic.
