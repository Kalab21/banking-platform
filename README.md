...
**Engineering proof:** 13 backend processes · 950 CI tests · idempotent money movement ·
...
| **Testing** | 950 tests in CI (JUnit 5, Mockito, Testcontainers, Vitest, Playwright), plus 34 live-stack Playwright scenarios and a PowerShell full-stack suite on demand |
...
| Evidence | Result |
|---|---:|
| Backend — unit, web-slice and Testcontainers integration | 668 |
...
The backend total is 554 unit and web-slice tests plus 114 integration tests that run
...
```bash
mvn -B --no-transfer-progress clean verify   # backend: 554 unit + 114 integration = 668
