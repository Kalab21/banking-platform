...
**Engineering proof:** 13 backend processes · 962 CI tests · idempotent money movement ·
...
| **Testing** | 962 tests in CI (JUnit 5, Mockito, Testcontainers, Vitest, Playwright), plus 34 live-stack Playwright scenarios and a PowerShell full-stack suite on demand |
...
| Evidence | Result |
|---|---:|
| Backend — unit, web-slice and Testcontainers integration | 680 |
...
The backend total is 554 unit and web-slice tests plus 126 integration tests that run
...
```bash
mvn -B --no-transfer-progress clean verify   # backend: 554 unit + 126 integration = 680
