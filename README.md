...
**Engineering proof:** 13 backend processes · 899 CI tests · idempotent money movement ·
...
| **Testing** | 899 tests in CI (JUnit 5, Mockito, Testcontainers, Vitest, Playwright), plus 34 live-stack Playwright scenarios and a PowerShell full-stack suite on demand |
...
| Evidence | Result |
|---|---:|
| Backend — unit, web-slice and Testcontainers integration | 617 |
...
The backend total is 547 unit and web-slice tests plus 70 integration tests that run
...
```bash
mvn -B --no-transfer-progress clean verify   # backend: 547 unit + 70 integration = 617
